...
def part1_smol(filename):
    with open(filename) as f:
        print(sum(list(map(lambda expression: prod([int(num) for num in expression.lstrip("mul(").rstrip(")").split(",")]),re.findall(r"mul\(\d{1,3},\d{1,3}\)",[line for line in f][0])))))
